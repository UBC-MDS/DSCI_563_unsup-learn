...
def plot_pca_regression(data, w, h, error_type='both'):
    ...
    if np.rad2deg(theta) <= 90:
        ax.plot([np.min((v[:, 0], x_std[:,0])), np.max((v[:, 0], x_std[:,0]))],
                   [m*np.min((v[:, 0], x_std[:,0])), m*np.max((v[:, 0], x_std[:,0]))], color="green", label='model')
    else:
        ax.plot([np.min((v[:, 0], x_std[:,0])), np.max((v[:, 0], x_std[:,0]))],
                   [-m*np.min((v[:, 0], x_std[:,0])), -m*np.max((v[:, 0], x_std[:,0]))], color="green", label='model')
    ...
    ax.set_title("PCA vs Regression", fontsize=14)
    ax.legend()
    plt.show()
...
def plot_clust(X,W=None,z=None, size=50):
    if z is not None:
        if np.any(z<0):
            plt.scatter(X[z<0,0], X[z<0,1], marker="o", 
                        facecolors='none', edgecolor='black', s=size, alpha=0.6);
        if np.any(z>=0):
            plt.scatter(X[z>=0,0], X[z>=0,1], marker="o", c=z[z>=0], s=size, alpha=0.6);
    else:
        plt.scatter(X[:,0], X[:,1], marker="o", s=size);
    if W is not None:
        plt.scatter(W[:,0], W[:,1], marker="^", s=size, c=np.arange(W.shape[0]));
    else:
